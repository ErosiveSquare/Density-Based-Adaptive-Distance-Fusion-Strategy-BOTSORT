...
class StableBoTSORT(BaseTracker):
    def __init__(
            self,
            model_weights,
            device,
        ...
        self.stable_history_length = 200  # 需要检查的历史长度
    ...
    def update(self, dets: np.ndarray, img: np.ndarray, embs: np.ndarray = None) -> np.ndarray:
        ...
        """ Step 4: Init new stracks 2025.1.26修改后"""
        # 在初始化新轨迹前进行稳定轨迹匹配
        ...
        for inew in u_detection:
            ...
            for stable in stable_tracks:
                iou = self.calculate_iou(track.xyxy, stable.xyxy)
                if iou > self.match_iou_thresh and iou > max_iou:
                    max_iou = iou
                    best_match = stable
            ...
            if best_match is not None:
                if self.with_reid:
                    emb_dist = embedding_distance([track], [best_match])[0][0]
                    if emb_dist < self.appearance_thresh:
                        matched_pairs.append((track, best_match))

            # 处理匹配到的稳定轨迹
        for new_track, stable_track in matched_pairs:
            # 传递ID并更新稳定轨迹
            new_track.id = stable_track.id
            stable_track.update(new_track, self.frame_count)
            activated_starcks.append(stable_track)
            # 从待激活列表中移除新轨迹（避免重复）
            if new_track in detections:
                detections.remove(new_track)
